...
public:
    ...
    template <size_t M>
    CUDA_HOST_DEVICE MatT<T, R, M> operator*(const MatT<T, C, M> &rhs) const {
        MatT<T, R, M> res = Zero();
    ...
    template <size_t M>
    CUDA_HOST_DEVICE MatT<T, R, M> &operator*=(const MatT<T, C, M> &rhs) {
        return *this = *this * rhs;
    }

    CUDA_HOST_DEVICE VecT<T, R> operator*(const VecT<T, C> &rhs) const {
    ...
    CUDA_HOST_DEVICE MatT<T, C, R> Transpose() const {
        MatT<T, C, R> res;
        for (size_t i = 0; i < R; i++) {
            for (size_t j = 0; j < C; j++) {
                res[i][j] = cols_[j][i];
    ...
    CUDA_HOST_DEVICE MatT<T, R, C> Inverse() const {
        ...
        for (size_t i = 0; i < C; i++) {
            ...
            if (i != pivot) {
                for (size_t j = 0; j < R; j++) {
                    T swap_temp = temp[j][i];
                    temp[j][i] = temp[j][pivot];
                    temp[j][pivot] = swap_temp;
                    swap_temp = res[j][i];
                    res[j][i] = res[j][pivot];
                    res[j][pivot] = swap_temp;
                }
            }
...
using CMat2 = details::MatT<unsigned char, 2, 2>;
...
using CMat3 = details::MatT<unsigned char, 3, 3>;
...
using CMat4 = details::MatT<unsigned char, 4, 4>;
